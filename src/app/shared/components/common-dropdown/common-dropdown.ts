...
export class CommonDropdown {
  ...
  protected onOptionChange(option: any): void {
    ...
    if (this.selectedOption && !this.disableSameSelect) {
      if (this.uniqueKey && this.selectedOption[this.uniqueKey] === option[this.uniqueKey]) isSameSelect = true;
      else if (this.selectedOption === option) isSameSelect = true;
    }
    // if the option is the same as the selected option and the `enableSameSelectRemove` is true, emit null
