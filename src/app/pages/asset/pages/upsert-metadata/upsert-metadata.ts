...
@Component({
  selector: 'app-upsert-asset-metadata',
  imports: [
    ReactiveFormsModule,
    AppSrc,
    FileDrop
...
export class UpsertMetadata {
  ...
  private _setImageFile(file: File): void {
    if (!file.type?.startsWith('image/')) {
      this._coreService.utils.showToaster(EToasterType.Warning, 'Please select a valid image file');
      return;
    }
  ...
  private _resolveResortId(): string {
    const resort = this.data?.resortId;
    if (typeof resort === 'string') return resort;
    if (resort?._id) return resort._id;
    if (typeof this.data?.propertyId?.resortId === 'string') return this.data.propertyId.resortId;
    if (this.data?.propertyId?.resortId?._id) return this.data.propertyId.resortId._id;
    return '';
  }
  ...
  protected onSubmit(): void {
    ...
    const resortId = this._resolveResortId();
    if (!resortId) {
      this._coreService.utils.showToaster(EToasterType.Danger, 'Resort not found for this asset');
      return;
    }
    ...
    const didTextChange = slug !== (prev.slug || '').trim() ||
    ...
    this._apiFs.asset.updateMetadata(this.data._id, body).subscribe({
      next: (res: IResponse) => {
        this.isReqAlive = false;
        if (res.code === 'UPDATED') {
          this._coreService.utils.showToaster(EToasterType.Success, 'Asset metadata updated successfully');
